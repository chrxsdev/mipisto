...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-border/60 transition-colors hover:bg-foreground/[0.028] data-[state=selected]:bg-muted",
        className
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-12 px-4 text-left align-middle text-[0.76rem] font-semibold tracking-[0.14em] whitespace-nowrap uppercase text-muted-foreground [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
